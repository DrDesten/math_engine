...
const constants_multimatch = [[3.141592653589793, "π", "pi", "Pi"], [2.718281828459045, "e", "e", "Euler's number"], [1.618033988749895, "φ", "phi", "Phi, Golden ratio"], [9.869604401089358, "π²", false, false], [7.3890560989306495, "e²", false, false], [2.618033988749895, "φ²", false, false], [0.3183098861837907, "π⁻¹", false, false], [0.36787944117144233, "e⁻¹", false, false], [0.6180339887498948, "φ⁻¹", false, false], [1.7724538509055159, "√π", false, false], [1.6487212707001282, "√e", false, false], [1.272019649514069, "√φ", false, false]]
...
function rationalizeConstants( x, maxDenominator = 32 ) {
    ...
    for ( let denom = 1; ( denom <= maxDenominator && error > 0 ); denom++ ) {
        ...
        let errRoots = checkConstants.map( check => Math.abs( Math.round( check * denom ) - ( check * denom ) ) )
        let errRootInverses = checkConstantInverses.map( check => Math.abs( Math.round( check * denom ) - ( check * denom ) ) )
