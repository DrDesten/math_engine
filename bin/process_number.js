...
function rationalizeConstants( x, maxDenominator = 3600 ) {
    ...
    const constants = [[3.141592653589793, "π"], [2.718281828459045, "e"], [1.618033988749895, "φ"], [9.869604401089358, "π²"], [7.3890560989306495, "e²"], [2.618033988749895, "φ²"], [0.3183098861837907, "π⁻¹"], [0.36787944117144233, "e⁻¹"], [0.6180339887498948, "φ⁻¹"], [1.7724538509055159, "√π"], [1.6487212707001282, "√e"], [1.272019649514069, "√φ"]]
        ...
        for ( let i = 0; i < errRoots.length; i++ ) {
            if ( errRoots[i] < error || errRootInverses[i] < error ) {
                ...
                let fraction = errRootInverses[i] < errRoots[i] ?
                    [Math.round( checkRootInverses[i] * denom ), denom] :
                    [Math.round( checkRoots[i] * denom ), denom]
                rootFractions.push( [
                    fraction[0],
                    fraction[1],
                    roots[i][1],
